  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !result.success) {
        setErrorMessage(result.message || 'Gagal membuat akun. Silakan coba lagi.');
        setIsLoading(false);
        return;
      }
      ...
      <div className="w-full max-w-[420px] bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 rounded-[10px] shadow-xl shadow-slate-200/50 dark:shadow-slate-950/50 p-7 sm:p-9 relative z-10">
        ...
        <div className="text-center mb-7">
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-tight">
            Daftar <span className="text-navy-700 dark:text-blue-400">Employr</span>
          </h1>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            Buat akun baru untuk mulai mengoptimalkan CV dan lamaran kerja kamu.
          </p>
        </div>
        ...
        <form onSubmit={handleRegister} className="flex flex-col gap-4 mb-5">
          <div className="flex flex-col gap-1.5 text-left">
            <label htmlFor="register-name-input" className="text-xs font-semibold text-slate-600 dark:text-slate-400">
              Nama Lengkap
            </label>
            <input
          ...
          <div className="flex flex-col gap-1.5 text-left">
            <label htmlFor="register-email-input" className="text-xs font-semibold text-slate-600 dark:text-slate-400">
              Email
            </label>
            <input
          ...
          <div className="flex flex-col gap-1.5 text-left">
            <label htmlFor="register-password-input" className="text-xs font-semibold text-slate-600 dark:text-slate-400">
              Kata Sandi
            </label>
            <input
