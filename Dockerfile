...
FROM base AS deps
COPY . .
RUN pnpm install --frozen-lockfile --filter @employr/api... --filter @employr/admin... --filter @employr/dashboard...
...
RUN pnpm --filter @employr/db exec prisma generate
RUN pnpm --filter @employr/api build
...
RUN pnpm --filter @employr/admin build
RUN pnpm --filter @employr/dashboard build
