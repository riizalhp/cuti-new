...
/**
 * Scroll container for a thread from @smoothui/ai-conversation.
 *
...
export const AIConversation = ({
  ...
  const scrollToBottom = useCallback((behavior: ScrollBehavior) => {
    ...
    // Element.scrollTo fallback for environments where smooth scrolling isn't natively supported
      ...
      <AnimatePresence initial={false}>
        {!isPinned && (
          <motion.button
            ...
            className="absolute inset-x-0 bottom-3 mx-auto flex w-fit cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 py-1.5 pr-3 pl-2.5 text-slate-700 dark:text-slate-200 text-xs font-semibold shadow-md backdrop-blur z-20 hover:bg-slate-100 dark:hover:bg-slate-800 transition"
            exit={
              shouldReduceMotion
            ...
            type="button"
          >
            <ArrowDown aria-hidden="true" size={13} className="text-[#1738D1]" />
            Ke pesan terbaru
          </motion.button>
        )}
      </AnimatePresence>
