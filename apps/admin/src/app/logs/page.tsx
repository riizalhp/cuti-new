    ...
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 mb-6 shadow-sm">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
          ...
          {/* Client Errors Quick Filter */}
          <button
            type="button"
            onClick={() => {
              if (search === "[Client Browser Error]") {
                setSearch("")
              } else {
                setSearch("[Client Browser Error]")
                setLogType("app")
              }
              setPage(1)
            }}
            className={`px-3 py-2 rounded-xl text-xs font-bold transition-all border flex items-center gap-1.5 cursor-pointer shrink-0 ${
              search === "[Client Browser Error]"
          ...
          >
            <AlertTriangle size={13} className={search === "[Client Browser Error]" ? "text-white" : "text-rose-500"} />
